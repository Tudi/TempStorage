...
int WriteWavHeader(const char* FileName, int AudioFormat, int channels, int bitsPerSample, int SampleRate, int FrameCount)
{
 ...
 //   channels = FORCE_OUTPUT_CHANNEL_COUNT;
    ...
    FILE* audio_dst_file;
    errno_t er = fopen_s(&audio_dst_file, FileName, "wb");
    if (audio_dst_file == NULL)
    {
        printf("Could not open Audio output file for writing\n");
        return -1;
    }

    write_count = fwrite(&wav_header, sizeof(wavfile_header_t), 1, audio_dst_file);
...
{
    FILE* audio_dst_file;
    errno_t er = fopen_s(&audio_dst_file, FileName, "ab");
    if (audio_dst_file == NULL)
    {
        printf("Could not open Audio output file for writing\n");
        return -1;
    }
    fwrite(buf, 1, Size, audio_dst_file);
    fclose(audio_dst_file);
    return 0;
}