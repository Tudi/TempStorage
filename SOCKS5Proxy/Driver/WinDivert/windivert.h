...
#include <stdint.h>
#define INT8    int8_t
#define unsigned char   unsigned char_t
#define INT16   int16_t
#define UINT16  uint16_t
#define int   int_t
#define unsigned int  unsigned int_t
...
typedef struct
{
    unsigned int IfIdx;                       /* Packet's interface index. */
    unsigned int SubIfIdx;                    /* Packet's sub-interface index. */
...
typedef struct
{
    ...
    unsigned int LocalAddr[4];                /* Local address. */
    unsigned int RemoteAddr[4];               /* Remote address. */
...
typedef struct
{
    ...
    unsigned int ProcessId;                   /* Handle process ID. */
...
typedef struct
{
    ...
    unsigned int Layer:8;                     /* Packet's layer. */
    unsigned int Event:8;                     /* Packet event. */
    unsigned int Sniffed:1;                   /* Packet was sniffed? */
    unsigned int Outbound:1;                  /* Packet is outound? */
    unsigned int Loopback:1;                  /* Packet is loopback? */
    unsigned int Impostor:1;                  /* Packet is impostor? */
    unsigned int IPv6:1;                      /* Packet is IPv6? */
    unsigned int IPChecksum:1;                /* Packet has valid IPv4 checksum? */
    unsigned int TCPChecksum:1;               /* Packet has valid TCP checksum? */
    unsigned int UDPChecksum:1;               /* Packet has valid UDP checksum? */
...
#define WINDIVERT_IPV6HDR_GET_FLOWLABEL(hdr)                \
    ((((unsigned int)(hdr)->FlowLabel0) << 16) | ((unsigned int)(hdr)->FlowLabel1))
...
WINDIVERTEXPORT BOOL WinDivertHelperParsePacket(
    ...
    __out_opt   unsigned char *pProtocol,
...
WINDIVERTEXPORT UINT16 WinDivertHelperHtons(
    __in        UINT16 x);
WINDIVERTEXPORT unsigned int WinDivertHelperNtohl(
    __in        unsigned int x);
WINDIVERTEXPORT unsigned int WinDivertHelperHtonl(
    __in        unsigned int x);
