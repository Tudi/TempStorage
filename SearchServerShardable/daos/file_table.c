...
int getFileTableEntry(FILE* file, DaosFileVersion_t daosVersion, DaosId_t id,
    DaosOffset_t* itemOffsetInFile, DaosSize_t* itemSize)
{
    rewind(file);
    ...
    if(fileVersion != daosVersion)
    {
        LOG_MESSAGE(ATTENTION_LOG_MSG, "Error: (id = %lu) - file version (%hu) != DAOS version (%hu).",
            id, fileVersion, daosVersion);
        return 2;
    }
    ...
    {
        LOG_MESSAGE(ATTENTION_LOG_MSG, "Error: (id = %lu) - fseek(item entry in table, offset = %zu, "
            "SEEK_CUR) failed.", id, itemOffsetInTable);
        return 5;
    }
    ...
    if(numBytesRead != sizeof(*itemOffsetInFile))
    {
        LOG_MESSAGE(ATTENTION_LOG_MSG, "Error: (id = %lu) - fread(itemOffsetInFile) failed. "
            "numBytesRead (%zu) != itemOffsetInFile size (%zu).", id, numBytesRead, sizeof(*itemOffsetInFile));
        return 6;
    }
...
int getFileTableVersion(FILE* file, DaosFileVersion_t* fileVersion)
{
    rewind(file);
...
int getFileTable(FILE* file, DaosFileVersion_t daosVersion, DaosFileVersion_t* fileVersion, DaosCount_t* capacity,
    DaosCount_t* numItemsInFile, FileTableEntry_t** fileTable)
{
    ...
    if(*fileVersion != daosVersion)
    {
        LOG_MESSAGE(ATTENTION_LOG_MSG, "Error: file version (%hu) != DAOS version (%hu).", *fileVersion, daosVersion);
        return 2;
    }
    ...
    if(fileTable != NULL)
    {
        *fileTable = (FileTableEntry_t*) malloc(*numItemsInFile * sizeof(FileTableEntry_t));
        ...
        for(DaosCount_t capacityIndex = 0; capacityIndex < *capacity; ++capacityIndex)
        {
            ...
            {
                LOG_MESSAGE(ATTENTION_LOG_MSG, "Error: fread(itemOffset) failed. table entry %lu / %lu. numBytesRead"
                    " (%zu) != itemOffset size (%zu).", capacityIndex, *capacity, numBytesRead, sizeof(itemOffset));
                return 6;
            }
            ...
            if(numBytesRead != sizeof(itemSize))
            {
                LOG_MESSAGE(ATTENTION_LOG_MSG, "Error: fread(itemSize) failed. table entry %lu / %lu. numBytesRead"
                    " (%zu) != itemSize's size (%zu).", capacityIndex, *capacity, numBytesRead, sizeof(itemSize));
                return 7;
            }
...
int writeInitialFileTableToFile(FILE* file, DaosFileVersion_t daosVersion, DaosCount_t capacity)
{
    // Write file version.

    size_t numBytesWritten = fwrite(&daosVersion, 1, sizeof(daosVersion), file);
    if(numBytesWritten != sizeof(daosVersion))
    {
        LOG_MESSAGE(ATTENTION_LOG_MSG, "Error: fwrite(version) failed. numBytesWritten (%zu) != version size (%zu).",
            numBytesWritten, sizeof(daosVersion));
        return 1;
    }
    ...
    DaosOffset_t nextOffset = sizeof(daosVersion) + sizeof(capacity) + sizeof(numItemsInFile)
...
int updateFileTableInFile(FILE* file, DaosFileVersion_t daosVersion, DaosId_t id, DaosSize_t itemSize)
{
    rewind(file);
    ...
    if(numBytesRead != sizeof(capacity))
    {
        LOG_MESSAGE(ATTENTION_LOG_MSG, "Error: (id = %lu) - fread(capacity) failed. numBytesRead (%zu)"
            " != capacity size (%zu).", id, numBytesRead, sizeof(capacity));
        return 3;
    }
    ...
    DaosCount_t numEntries, DaosCalculation_t sizeDifference)
{
    ...
    { 
        ...
        DaosCalculation_t auxItemOffset = (DaosCalculation_t) *itemOffset + (DaosCalculation_t) sizeDifference;
