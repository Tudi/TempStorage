...
static int writeItemToFile(FILE* file, const ItemFunctions_t* itemFunctions, const void* item, DaosSize_t* itemSize);
...
int processItemRead(FILE* file, const uint8_t* buffer, DaosSize_t itemSize, BinaryToItem_t binaryToItem, void** item, int fileVersion)
{
    // Header
    ...
    DaosSize_t itemDataSize = itemSize - sizeof(recordHeader) - sizeof(recordTrailer);

    int ret = binaryToItem(offset, item, fileVersion);
    if(ret != 0)
    {
        LOG_MESSAGE(ATTENTION_LOG_MSG, "Error: binaryToItem() returned %d.", ret);
        return 3;
    } 
...
static int writeItemToFile(FILE* file, const ItemFunctions_t* itemFunctions, const void* item, DaosSize_t* itemSize)
{
    ...
    if(item != NULL)
    {
        uint8_t* result = itemFunctions->persistentItemToBinary(itemBinary, item);
        if(result != itemBinary + itemBinarySize)
        {
            LOG_MESSAGE(ATTENTION_LOG_MSG, "Error: itemToBinary(id = %lu) failed.",
                itemFunctions->getPersistentItemId(item));
            return 3;
        } 

        numBytesWritten = fwrite(itemBinary, 1, itemBinarySize, file);
