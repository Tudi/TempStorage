...
#define LOG_MSG_MAX_LENGTH 8096
    ...
    const char* filename, int lineNumber, ...)
{
    ...
    numCharsWritten = vsnprintf(&logMsg[msgStartPos], logMsgSize, format, varArgs);
    ...
    if((numCharsWritten > 0) && (numCharsWritten < logMsgSize))
    {
        ...
        logMsgSize -= numCharsWritten;
        snprintf(&logMsg[msgStartPos], logMsgSize, " (%s:%d)", filename, lineNumber);
    }
    ...
    const char* functionName)
{
    ...
    size_t numCharsWritten = strftime(logMsg, logMsgSize, "%F %T", &tms);
