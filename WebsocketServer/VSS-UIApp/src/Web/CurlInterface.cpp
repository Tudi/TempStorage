...
void CURL_AsyncExecuteThread(WatchdogThreadData* wtd, CURL_WorkerThreadParams *wtp)
{
    ...
    while (sAppSession.IsApplicationRunning() == true && wtd->ShouldShutDown() == false)
    {
        ...
        if (wtp->CURLJobQueue.empty()) {
            ...
            // this stat will also monitor processing duration
            sKPI.UpdateAsyncDurationStat(GetTickCount64() - job->queueStamp);
...
void TestCurlWorksCallback(int CurlErr, char* response, void* userData)
{
    userData;
...
void TestCurlWorks()
{
    char* resp = GetURLResponse("https://www.google.com/");
    InternalFree(resp);

    GetURLResponseAsync("https://www.google.com/", NULL, TestCurlWorksCallback, NULL);
}