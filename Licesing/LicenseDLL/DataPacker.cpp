...
int GenericDataStore::PushData(const void *buff, int Size, int Type)
{
	...
	else
	{
		...
		Store->Size = (unsigned int)Size;
		Store->Type = (unsigned char)Type;
...
int DataCollectionIterator::GetNext(char **Data, int &Size, int &Type)
{
	...
	*Data = (char*)DataHeader->Data;
	Size = (int)DataHeader->Size; 
	Type = (int)DataHeader->Type;
...
int	GenericDataStore::IsDataValid()
{
	...
	{
		Log(LL_ERROR, DP_DATA_INTEGRITY_FAILURE, "Error : Internal data integrity failure");
		return 0;
	}
...
int	GenericDataStore::MergeLicenseNodesFrom(GenericDataStore *src)
{
	if (src == NULL)
		return ERROR_MISSING_PARAMETER;

	int retError = 0;
	//iterate src content and add it to us
	DataCollectionIterator itr;
	itr.Init(src);
	char *tData;
	int tSize;
	...
	int tErr;
	while (DCI_SUCCESS == itr.GetNext(&tData, tSize, tType))
		if (tType == DB_LICENSE_NODE  && (tErr = PushData(tData, tSize, tType)) != 0)
			retError = tErr;

	return retError;
}

int	GenericDataStore::HasNode(const void *buff, int Size, int Type)
{
	//sanity checks
	if (Data == NULL || Size == NULL)
		return ERROR_BAD_ARGUMENTS;
	...
	int tType;
	while (DCI_SUCCESS == itr.GetNext(&tData, tSize, tType))
		if (Type == tType && tSize == Size)
		{
			if (memcmp(tData, buff, Size) == 0)
				return 1;
		}
...
int	GenericDataStore::GetNodeCount()
{
	//search for the field
	DataCollectionIterator itr;
	itr.Init(this);
