...
FILE *FCONTENT2 = NULL;
void DumpContentFull(unsigned char *data, unsigned int size)
{
	if (FCONTENT2 == NULL)
		errno_t er = fopen_s(&FCONTENT2, "p_full", "wb");
	if (FCONTENT2)
	{
		fwrite(data, 1, size, FCONTENT2);
		fflush(FCONTENT2);
	}
}
FILE *FCONTENT3 = NULL;
void DumpContentMaybeFragmented(unsigned char *data, unsigned int size)
{
	if (FCONTENT3 == NULL)
		errno_t er = fopen_s(&FCONTENT3, "p_frag", "wb");
	if (FCONTENT3)
	{
		fwrite(&size, 1, sizeof(size), FCONTENT3);
		fwrite(data, 1, size, FCONTENT3);
		fflush(FCONTENT3);
	}
}

#define IP(a,b,c,d) (((unsigned int)a<<24)|((unsigned int)b<<16)|((unsigned int)c<<8)|(unsigned int)d)

static int FirstIPSaw = 0;
static int PrevAck2 = -1;
...
//	printf("%s.%.6d len:%d ", timestr, header->ts.tv_usec, header->len);
	...
	//debugging as always
	if (ih->ip_protocol == 6 && FirstIPSaw == 0)
		FirstIPSaw = ih->ip_srcaddr;

	//capturing all TCP packets from IP
	if (ih->ip_protocol == 6 
		//&& ( ih->ip_srcaddr == IP(192, 243, 47, 118) || ih->ip_srcaddr == IP(118, 47, 243, 1923) )
