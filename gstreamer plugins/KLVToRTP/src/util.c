...
GstBuffer* wrap_in_rtp(GstMyElement* self, GstBuffer* raw_buf) {
    ...
    guint64 timeNow = get_time_ms();
    guint64 timeDiff = timeNow - self->stampPrevSrcPacket;
    ...
    self->stampPrevSrcPacket = timeNow;
...
{
    if (!input || !out_entries || !out_count)
        return FALSE;

    gchar** tokens = g_strsplit(input, ",", 0);
    if (!tokens)
        return FALSE;
    ...
    *out_count = 0;
    ...
    // unexpected but we received 0 values
    if ((*out_count) == 0)
    {
        return FALSE;
    }

    // alloc mem to store the keys
    *out_entries = calloc(1, sizeof(KLVFilterEntry) * (*out_count));
    if ((*out_entries) == NULL)
    {
        *out_count = 0;
        return FALSE;
    ...
    // copy string value as numeric values
    guint64 validIndexCounter = 0;
    for (guint i = 0; tokens[i] != NULL; ++i) {
        gchar* trimmed = g_strstrip(tokens[i]);
        if (*trimmed == '\0')
            continue;  // skip empty tokens
        ...
        gchar* endptr = NULL;
        guint64 val = g_ascii_strtoull(trimmed, &endptr, 10); // decimal
        if (endptr == trimmed || *endptr != '\0') {
            g_warning("Invalid KLV key: '%s'", trimmed);
            continue;  // skip malformed entries
        }

        (*out_entries)[validIndexCounter].keyVal = val;
        (*out_entries)[validIndexCounter].timestampNextSend = 0;
        validIndexCounter++;
    }
    // make sure to refresh this value
    *out_count = validIndexCounter;
...
GstBuffer* filter_klv_buffer(GstMyElement* self, GstBuffer* buf) {
    ...
    if (!gst_buffer_map(buf, &map, GST_MAP_READ)) {
        GST_ERROR_OBJECT(self, "Failed to map buffer for KLV filtering");
        return NULL;
    }
    ...
    // Create a new buffer for filtered output
    GstBuffer* filtered_buf = gst_buffer_new();
    GstMapInfo filtered_map;
    gst_buffer_append_memory(filtered_buf, gst_allocator_alloc(NULL, len, NULL)); // max possible size

    if (!gst_buffer_map(filtered_buf, &filtered_map, GST_MAP_WRITE)) {
        GST_ERROR_OBJECT(self, "Failed to map filtered buffer");
        gst_buffer_unmap(buf, &map);
        gst_buffer_unref(filtered_buf);
        return NULL;
    }

    size_t in_offset = 0, out_offset = 0;
    // if no filter has been specified, we allow all. Noob proof
    if (self->KLVsAllowedCount == 0)
    {
        memcpy(&filtered_map.data[0], &input[0], len);
        out_offset = len;
    }
    // only allow to pass through specified list at limited rate
    else
    {
        guint64 timeNow = get_time_ms();

        while (in_offset + 2 < len) { // Need at least key(1) + len(1)
            uint8_t key = input[in_offset];
            uint8_t val_len = input[in_offset + 1];

            if (in_offset + 2 + val_len > len)
            {
                break; // malformed. could be simply that UDP packet got cut in half
            }

            // Check if this key is allowed
            gboolean allowed = FALSE;
            for (size_t i = 0; i < self->KLVsAllowedCount; ++i) {
                if (self->KLVsAllowed[i].keyVal == key && timeNow >= self->KLVsAllowed[i].timestampNextSend) {
                    self->KLVsAllowed[i].timestampNextSend = timeNow + 60 * 1000 / self->max_upm;
                    allowed = TRUE;
                    break;
                }
            }
            ...
            if (allowed) {
                filtered_map.data[out_offset++] = key;
                filtered_map.data[out_offset++] = val_len;
                memcpy(&filtered_map.data[out_offset], &input[in_offset + 2], val_len);
                out_offset += val_len;
            }
    ...
    gst_buffer_unmap(buf, &map);
    gst_buffer_unmap(filtered_buf, &filtered_map);
    gst_buffer_resize(filtered_buf, 0, out_offset); // trim unused space
