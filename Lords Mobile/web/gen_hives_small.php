...
for( $y=$RadiusHalf;$y<$MaxY-$RadiusHalf;$y+=$RadiusHalf*2)
	for( $x=$RadiusHalf;$x<$MaxX-$RadiusHalf;$x += $RadiusHalf*2)
	{
		...
		$query1 = "select guild,x,y from players where k=$k and x>=".($x-$RadiusHalf)." and x<=".($x+$RadiusHalf)." and y>=".($y-$RadiusHalf)." and y<=".($y+$RadiusHalf)."";		
...
//echo "$query2<br>"				;
				...
				if($pcount>=$MinPCount)
				{
					...
					$avg_plevel = (int)($plevel / $pcount);
//					echo "could be a minihive for $name at $avg_x $avg_y with $pcount players, $might, $kill<br>";
					$query2 = "insert into guild_hives_multi (x,y,gname,might,kills,plevel,clevel,pcount)values($avg_x,$avg_y,'".mysql_real_escape_string($name)."',$might,$kill,$avg_plevel,$avg_clevel,$pcount)";		
					$result2 = mysql_query($query2,$dbi) or die("Error : 20170220042 <br>".$query2." <br> ".mysql_error($dbi));
				}
			}
	}
