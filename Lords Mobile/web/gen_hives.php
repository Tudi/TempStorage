...
foreach( $KList as $key => $k)
{
	...
	foreach( $GuildList as $key => $guild)
	{
		...
		$query1 = "select x,y,might from players where k=$k and guild like '".mysql_real_escape_string($tguild)."'";		
		$result1 = mysql_query($query1,$dbi) or die("Error : 2017022004 <br>".$query1." <br> ".mysql_error($dbi));
		unset($Guildx);
		unset($Guildy);
		unset($Guildmight);
		...
		$playercount = 0;
		$TotalMight = 0;
		while( list( $x,$y, $might ) = mysql_fetch_row( $result1 ))
		{
...
//exit();
		$query1 = "insert into guild_hives (k,x,y,guild,radius,HiveCastles,TotalCastles,HiveMight,TotalMight)values($k,$xavg_prev,$yavg_prev,'".mysql_real_escape_string($guild)."',$MaxDist,$cordcount,$playercount,$mightsum,$TotalMight)";		
		$result1 = mysql_query($query1,$dbi) or die("Error : 2017022004 <br>".$query1." <br> ".mysql_error($dbi));
	}	
}
?>