...
//            if( FuncCallState[namecrc] == CALL_STATE_STARTED )    //recursive calls are possible
            {
                ...
                __int64 CallDiff2 = Context->FuncCallDepthStart[ Context->CallDepth - 1 ];
...
void PrintReport()
{
    ...
    TCHAR szFileName[MAX_PATH];
    DWORD ret = GetModuleFileName( NULL, szFileName, MAX_PATH );
    //truncate the EXE name
    int StrLen = strlen( szFileName );
    if( StrLen > 4 )
    {
        szFileName[ StrLen - 4 ] = 0;
        std::string str = szFileName;
        int SlashPos = str.find_last_of("/\\") + 1;
        if( SlashPos < StrLen && SlashPos > 0 ) 
            strcpy( szFileName, &szFileName[ SlashPos ] ); 
    }
    else
        strcpy( szFileName, " " );

    sprintf_s( FileName, 500, "d:/temp/perf_report_%d_%s.txt", (int)GetCurrentProcessId(), szFileName );
    ...
    for( std::map<DWORD,ThreadDebugContext*>::iterator citr = ThreadsDebugged.begin(); citr != ThreadsDebugged.end(); citr++ )
    {
        ...
        fprintf( FileDebug, "Sum time spent : %d\n", ( long long )( (double)SumTicks / PCFreq ) );
        //order by time spent in func
        fprintf( FileDebug, "=======================================================================================================\n");
        fprintf( FileDebug, "% 160s % 10s % 10s % 20s\n", "Function name", "call count", "Time share %", "Time MS");
        for( std::vector<std::pair<int,__int64>>::iterator itr = FuncCallSumSorted.begin(); itr != FuncCallSumSorted.end(); itr++ )
        {
            float TimeShare = Context->FuncCallSum[itr->first] * 100.0f / (float)SumTicks;
            float MSTime = Context->FuncCallSum[itr->first] / PCFreq;
            fprintf( FileDebug, "% 160s % 10d % 10.2f % 20.2f\n",Context->FuncCallNames[itr->first], (int)Context->FuncCallCount[itr->first], TimeShare, MSTime );
        }
    }
    fclose( FileDebug );
}
