...
#define SHA1_BYTE_COUNT 20
#define SHA1_MAX_APPIDS_TRACKED	20 // could make it dynamic
...
typedef struct plugin_DB_row {
	uint8_t SHA1_certificate[SHA1_BYTE_COUNT]; // binary hash. Put it at the start of the struct to ensure allignment
	app_id_confidence apps[SHA1_MAX_APPIDS_TRACKED];
...
static void init_plugin_DB_row_SHA1(plugin_DB_row* this) {
...
static plugin_DB_row* g_SHA1_db_cache = NULL;
#ifdef _APP_MODE_BUILD_DB
...
static void init_plugin_mysql_table() {
	const char* create_SHA1_fingerprints_table_sql =
		"CREATE TABLE IF NOT EXISTS SHA1_fingerprints (\n"
		"    id INT UNSIGNED NOT NULL AUTO_INCREMENT,\n"
		"    SHA1_hash CHAR(41) NOT NULL,\n"
		"    occurrences INT UNSIGNED NOT NULL DEFAULT 1,\n"
		"    expires_at BIGINT UNSIGNED DEFAULT NULL,\n"
		"    application_id INT UNSIGNED DEFAULT NULL,\n"
		"    sources TEXT NOT NULL,\n"
		"    PRIMARY KEY (id),\n"
		"    CONSTRAINT fk_application_SHA1 FOREIGN KEY (application_id)\n"
		"        REFERENCES applications(id)\n"
		"        ON DELETE SET NULL\n"
		"        ON UPDATE CASCADE\n"
		") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;";

	if (get_mysql_conn() && mysql_query(get_mysql_conn(), create_SHA1_fingerprints_table_sql)) {
		AddLogEntryB(LDF_LOCAL, LogSeverityError, LogSourceMySQL, "query %s failed. Error: %s\n", 
			create_SHA1_fingerprints_table_sql, mysql_error(get_mysql_conn()));
	}
}

static int get_app_id_slot_SHA1(plugin_DB_row* existing_entry, uint16_t app_id) {
	if (existing_entry == NULL) {
		return 0;
	}
	for (size_t i = 0; i < SHA1_MAX_APPIDS_TRACKED; i++) {
		if (existing_entry->apps[i].application_id == app_id) {
			return i;
		}
	}
	if (app_id > 0) {
		AddLogEntryB(LDF_LOCAL, LogSeverityWarn, LogSourcePluginSHA1, "SHA1 : All app_id slots are taken. Increase SHA1_MAX_APPIDS_TRACKED\n");
	}
	return -1;
}

#ifdef _APP_MODE_BUILD_DB
static void on_new_entry_found_db_gen(const uint8_t* pSHA1_certificate, uint64_t SHA1_size, uint64_t expires_stamp) {
	(void)SHA1_size;
	// do we have this value ?
	plugin_DB_row* existing_entry;
	HASH_FIND(hh, g_SHA1_db_cache, pSHA1_certificate, SHA1_BYTE_COUNT, existing_entry);
	int existing_app_index = get_app_id_slot_SHA1(existing_entry, app_id_for_pcap);
	...
	if (existing_entry != NULL && existing_app_index >= 0) {
		if (existing_entry->updated_ocurrences == 0 && existing_entry->row_id) {
			...
			snprintf(update_sql, sizeof(update_sql), "update SHA1_fingerprints set occurrences = occurrences + 1 where id = %d", existing_entry->row_id);
			if (get_mysql_conn() && mysql_query(get_mysql_conn(), update_sql)) {
				AddLogEntryB(LDF_LOCAL, LogSeverityError, LogSourceMySQL, "query %s failed. Error: %s\n", update_sql, mysql_error(get_mysql_conn()));
			}
			snprintf(update_sql, sizeof(update_sql), "update SHA1_fingerprints set sources = CONCAT(sources, '%s,') where id = %d and sources NOT LIKE '%%%s,%%'",
	...
	if (existing_entry != NULL && existing_app_index == -1) {
		existing_app_index = get_app_id_slot_SHA1(existing_entry, 0);
	...
	if (existing_entry == NULL) {
		...
		init_plugin_DB_row_SHA1(new_entry);
		size_t copy_len = sizeof(new_entry->SHA1_certificate);
		memcpy(new_entry->SHA1_certificate, pSHA1_certificate, copy_len);
		new_entry->updated_ocurrences = 1;
		new_entry->apps[existing_app_index].application_id = app_id_for_pcap;
		HASH_ADD(hh, g_SHA1_db_cache, SHA1_certificate, SHA1_BYTE_COUNT, new_entry);
	}
	...
	if (new_entry != NULL && get_mysql_conn()) {
		...
		char hex[SHA1_BYTE_COUNT * 2 + 1];
		for (int i = 0; i < SHA1_BYTE_COUNT; i++)
			sprintf(hex + i * 2, "%02x", pSHA1_certificate[i]);
		hex[SHA1_BYTE_COUNT * 2] = '\0';
		snprintf(update_sql, sizeof(update_sql), "insert into SHA1_fingerprints "
			"(SHA1_hash,expires_at,application_id,sources) values ('%s',%ld,%d,'%s,')", hex, expires_stamp, new_entry->apps[existing_app_index].application_id, _pcap_file[0]);
		if (mysql_query(get_mysql_conn(), update_sql)) {
			AddLogEntryB(LDF_LOCAL, LogSeverityError, LogSourceMySQL, "query %s failed. Error: %s\n", update_sql, mysql_error(get_mysql_conn()));
		}
		else {
...
#else
static void on_new_entry_found_report(nDPI_pkt_parser_params* pktparams, uint8_t* pSHA1_certificate) {
	// do we have this value ?
	plugin_DB_row* existing_entry;
	HASH_FIND(hh, g_SHA1_db_cache, pSHA1_certificate, SHA1_BYTE_COUNT, existing_entry);
	// should only increase counter once per pcap file
	if (existing_entry != NULL) {
		...
		for (int i = 0; i < SHA1_BYTE_COUNT; i++)
			sprintf(hex + i * 2, "%02x", pSHA1_certificate[i]);
		hex[SHA1_BYTE_COUNT * 2] = '\0';
		AddLogEntryB(LDF_LOCAL, LogSeverityDebug, LogSourcePluginSHA1, "SHA1 identified flow %d as app %ld with confidence %f. SHA1: %s\n",
			pktparams->flow_to_process->flow_id, existing_entry->apps[0].application_id, existing_entry->apps[0].confidence, hex);
#else
		AddLogEntryB(LDF_LOCAL, LogSeverityDebug, LogSourcePluginSHA1, "SHA1 identified flow %d as app %ld with confidence %f\n", 
		...
		for (size_t i = 0; i < _countof(existing_entry->apps); i++) {
			if (existing_entry->apps[i].application_id != 0) {
				...
				queue_app_detected_result(pktparams, PT_SHA1_CERT, existing_entry->apps[i].application_id, existing_entry->apps[i].confidence);
			}
		}
	}
}
...
static void update_app_confidence_scores_SHA1()
{
	plugin_DB_row *entry = NULL, *tmp= NULL;

	HASH_ITER(hh, g_SHA1_db_cache, entry, tmp) {
		for (size_t i = 0; i < SHA1_MAX_APPIDS_TRACKED; i++) {
...
static void insert_update_db_row_to_cache_SHA1(const char *szid, const unsigned char *pSHA1_hash, const char *szapplication_id, const char *szoccurrences) {
	long app_id = strtol(szapplication_id, NULL, 10);
#ifndef _APP_MODE_BUILD_DB
	(void)szid;
#endif
	plugin_DB_row* existing_entry;
	HASH_FIND(hh, g_SHA1_db_cache, pSHA1_hash, SHA1_BYTE_COUNT, existing_entry);
	// should only increase counter once per pcap file
	if (existing_entry != NULL) {
#ifdef _APP_MODE_BUILD_DB
		...
		int free_slot = get_app_id_slot_SHA1(existing_entry, 0);
		if (free_slot >= 0) {
...
#endif
		size_t copy_len = sizeof(new_entry->SHA1_certificate);
		memcpy(new_entry->SHA1_certificate, pSHA1_hash, copy_len);
...
void init_packetp_SHA1() {
	g_is_plugin_disabled = get_ini_int_value("Plugins", "Disable_Certificate", 0);
	...
	if (get_mysql_conn()) {
		...
		const char* select_query = "select id,SHA1_hash,application_id,occurrences from SHA1_fingerprints";
		if (mysql_query(get_mysql_conn(), select_query)) {
			...
			if (res) {
				...
				while ((row = mysql_fetch_row(res))) {
					const char* hex_hash = row[1];
					unsigned char bin_hash[SHA1_BYTE_COUNT]; // result

					for (int i = 0; i < SHA1_BYTE_COUNT; i++) {
						char byte_str[3] = { hex_hash[i * 2], hex_hash[i * 2 + 1], '\0' };
						bin_hash[i] = (unsigned char)strtoul(byte_str, NULL, 16);
					}

					insert_update_db_row_to_cache_SHA1(row[0], bin_hash, row[2], row[3]);
					entries_loaded++;
				}
				mysql_free_result(res);
			}
		}
		AddLogEntryB(LDF_LOCAL, LogSeverityDebug, LogSourcePluginSHA1, "SHA1 : Loaded %ld entries\n", entries_loaded);

		// calculate confidence
		update_app_confidence_scores_SHA1();
	}
}
...
void destroy_packetp_SHA1() {
	...
	HASH_ITER(hh, g_SHA1_db_cache, entry, tmp) {
		HASH_DEL(g_SHA1_db_cache, entry); 
		free(entry);                      
	}
}
...
#define flow_user			   (pktparams->flow_to_process)
	...
	if (flow_user->checked_plugin_SHA1 != 0) {
		return RETC_NO_ERR;
	}
	if (g_is_plugin_disabled != 0) {
		return RETC_NO_ERR;
	}
	if (flow_user->detected_protocol.proto.master_protocol == NDPI_PROTOCOL_TLS ||
		flow_user->detected_protocol.proto.app_protocol == NDPI_PROTOCOL_TLS) {
//		if (flow_user->ndpi_flow && flow_user->ndpi_flow->protos.tls_quic.fingerprint_set != 0) {
		if (flow_user->sha1_cert_fingerprint_set == 1) {
			flow_user->checked_plugin_SHA1 = 1;
//			uint8_t* sha1 = flow_user->ndpi_flow->protos.tls_quic.sha1_certificate_fingerprint;
			uint8_t* sha1 = (uint8_t *)flow_user->ssh_tls.sha1_cert_fingerprint;
#ifdef _APP_MODE_BUILD_DB
			on_new_entry_found_db_gen(sha1, SHA1_BYTE_COUNT, flow_user->ssh_tls.notAfter);
#else
			on_new_entry_found_report(pktparams, sha1);
#endif
		}
	}
