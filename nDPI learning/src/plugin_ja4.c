...
#define JA4_BYTE_COUNT 37
#define JA4_MAX_APPIDS_TRACKED	20 // could make it dynamic
...
typedef struct plugin_DB_row {
	char ja4_client[JA4_BYTE_COUNT + 1]; // MD5 hash string + null terminator. Put it at the start of the struct to ensure allignment
	app_id_confidence apps[JA4_MAX_APPIDS_TRACKED];
...
static void init_plugin_DB_row_JA4(plugin_DB_row* this) {
...
static plugin_DB_row* g_ja4_db_cache = NULL;
#ifdef _APP_MODE_BUILD_DB
...
static void init_plugin_mysql_table() {
	const char* create_ja4_fingerprints_table_sql =
		"CREATE TABLE IF NOT EXISTS ja4_fingerprints (\n"
		"    id INT UNSIGNED NOT NULL AUTO_INCREMENT,\n"
		"    ja4_hash CHAR(38) NOT NULL,\n"
		"    occurrences INT UNSIGNED NOT NULL DEFAULT 1,\n"
		"    expires_at BIGINT UNSIGNED DEFAULT NULL,\n"
		"    application_id INT UNSIGNED DEFAULT NULL,\n"
		"    sources TEXT NOT NULL,\n"
		"    PRIMARY KEY (id),\n"
		"    CONSTRAINT fk_application_ja4 FOREIGN KEY (application_id)\n"
		"        REFERENCES applications(id)\n"
		"        ON DELETE SET NULL\n"
		"        ON UPDATE CASCADE\n"
		") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;";

	if (get_mysql_conn() && mysql_query(get_mysql_conn(), create_ja4_fingerprints_table_sql)) {
		AddLogEntryB(LDF_LOCAL, LogSeverityError, LogSourceMySQL, "query %s failed. Error: %s\n", 
			create_ja4_fingerprints_table_sql, mysql_error(get_mysql_conn()));
	}
}

static int get_app_id_slot_JA4(plugin_DB_row* existing_entry, uint16_t app_id) {
	if (existing_entry == NULL) {
		return 0;
	}
	for (size_t i = 0; i < JA4_MAX_APPIDS_TRACKED; i++) {
		if (existing_entry->apps[i].application_id == app_id) {
			return i;
		}
	}
	if (app_id > 0) {
		AddLogEntryB(LDF_LOCAL, LogSeverityWarn, LogSourcePluginJA4, "JA4 : All app_id slots are taken. Increase JA4_MAX_APPIDS_TRACKED\n");
	}
	return -1;
}

#ifdef _APP_MODE_BUILD_DB
static void on_new_entry_found_db_gen(char* ja4_client, uint64_t ja4_size, uint64_t expires_stamp) {
	// do we have this value ?
	plugin_DB_row* existing_entry;
	HASH_FIND(hh, g_ja4_db_cache, ja4_client, JA4_BYTE_COUNT, existing_entry);
	int existing_app_index = get_app_id_slot_JA4(existing_entry, app_id_for_pcap);
	...
	if (existing_entry != NULL && existing_app_index >= 0) {
		if (existing_entry->updated_ocurrences == 0 && existing_entry->row_id) {
			...
			snprintf(update_sql, sizeof(update_sql), "update ja4_fingerprints set occurrences = occurrences + 1 where id = %d", existing_entry->row_id);
			if (get_mysql_conn() && mysql_query(get_mysql_conn(), update_sql)) {
				AddLogEntryB(LDF_LOCAL, LogSeverityError, LogSourceMySQL, "query %s failed. Error: %s\n", update_sql, mysql_error(get_mysql_conn()));
			}
			snprintf(update_sql, sizeof(update_sql), "update ja4_fingerprints set sources = CONCAT(sources, '%s,') where id = %d and sources NOT LIKE '%%%s,%%'",
	...
	if (existing_entry != NULL && existing_app_index == -1) {
		existing_app_index = get_app_id_slot_JA4(existing_entry, 0);
	...
	if (existing_entry == NULL) {
		...
		init_plugin_DB_row_JA4(new_entry);
		size_t copy_len = ja4_size < sizeof(new_entry->ja4_client) - 1 ? ja4_size : sizeof(new_entry->ja4_client) - 1;
		memcpy(new_entry->ja4_client, ja4_client, copy_len);
		new_entry->ja4_client[copy_len] = '\0';
		new_entry->updated_ocurrences = 1;
		new_entry->apps[existing_app_index].application_id = app_id_for_pcap;
		HASH_ADD(hh, g_ja4_db_cache, ja4_client, JA4_BYTE_COUNT, new_entry);
	}
	...
	if (new_entry != NULL && get_mysql_conn()) {
		...
		snprintf(update_sql, sizeof(update_sql), "insert into ja4_fingerprints "
			"(ja4_hash,expires_at,application_id,sources) values ('%s',%ld,%d,'%s,')", ja4_client, expires_stamp, new_entry->apps[existing_app_index].application_id, _pcap_file[0]);
		if (mysql_query(get_mysql_conn(), update_sql)) {
			AddLogEntryB(LDF_LOCAL, LogSeverityError, LogSourceMySQL, "query %s failed. Error: %s\n", update_sql, mysql_error(get_mysql_conn()));
		}
		else {
...
#else
static void on_new_entry_found_report(nDPI_pkt_parser_params* pktparams, char* ja4_client) {
	// do we have this value ?
	plugin_DB_row* existing_entry;
	HASH_FIND(hh, g_ja4_db_cache, ja4_client, JA4_BYTE_COUNT, existing_entry);
	// should only increase counter once per pcap file
	if (existing_entry != NULL) {
		AddLogEntryB(LDF_LOCAL, LogSeverityDebug, LogSourcePluginJA4, "JA4 identified flow %d as app %ld with confidence %f. JA4C: %s\n", 
			pktparams->flow_to_process->flow_id, existing_entry->apps[0].application_id, existing_entry->apps[0].confidence, ja4_client);
		for (size_t i = 0; i < _countof(existing_entry->apps); i++) {
			if (existing_entry->apps[i].application_id != 0) {
				...
				queue_app_detected_result(pktparams, PT_JA4, existing_entry->apps[i].application_id, existing_entry->apps[i].confidence);
			}
		}
	}
}
...
static void update_app_confidence_scores_JA4()
{
	plugin_DB_row *entry = NULL, *tmp= NULL;

	HASH_ITER(hh, g_ja4_db_cache, entry, tmp) {
		for (size_t i = 0; i < JA4_MAX_APPIDS_TRACKED; i++) {
...
static void insert_update_db_row_to_cache_JA4(const char *szid, const char *szja4_hash, const char *szapplication_id, const char *szoccurrences) {
	long app_id = strtol(szapplication_id, NULL, 10);
#ifndef _APP_MODE_BUILD_DB
	(void)szid;
#endif
	plugin_DB_row* existing_entry;
	char db_row_key[JA4_BYTE_COUNT];
	memset(db_row_key, 0, sizeof(db_row_key));
	memcpy(db_row_key, szja4_hash, min(sizeof(db_row_key) - 1, strlen(szja4_hash)));
	HASH_FIND(hh, g_ja4_db_cache, db_row_key, JA4_BYTE_COUNT, existing_entry);
	// should only increase counter once per pcap file
	if (existing_entry != NULL) {
#ifdef _APP_MODE_BUILD_DB
		...
		int free_slot = get_app_id_slot_JA4(existing_entry, 0);
		if (free_slot >= 0) {
...
#endif
		uint64_t ja4_size = strlen(szja4_hash); // should always be 32
		size_t copy_len = ja4_size < sizeof(new_entry->ja4_client) - 1 ? ja4_size : sizeof(new_entry->ja4_client) - 1;
		memcpy(new_entry->ja4_client, szja4_hash, copy_len);
		new_entry->ja4_client[copy_len] = '\0';
...
void init_packetp_JA4() {
	g_is_plugin_disabled = get_ini_int_value("Plugins", "Disable_JA4C", 0);
	...
	if (get_mysql_conn()) {
		...
		const char* select_query = "select id,ja4_hash,application_id,occurrences from ja4_fingerprints";
		if (mysql_query(get_mysql_conn(), select_query)) {
			...
			if (res) {
				...
				while ((row = mysql_fetch_row(res))) {
					insert_update_db_row_to_cache_JA4(row[0], row[1], row[2], row[3]);
					entries_loaded++;
				}
				mysql_free_result(res);
			}
		}
		AddLogEntryB(LDF_LOCAL, LogSeverityDebug, LogSourcePluginJA4, "JA4 : Loaded %ld entries\n", entries_loaded);

		// calculate confidence
		update_app_confidence_scores_JA4();
	}
}
...
#define flow			   (pktparams->flow_to_process)
	...
	if (flow->detected_protocol.proto.master_protocol == NDPI_PROTOCOL_TLS ||
		...
		if (*(uint64_t*)&(flow->ssh_tls.ja4_client[0]) != 0) {
			flow->checked_plugin_JA4 = 1;
#ifdef _DEBUG
			_Static_assert(sizeof(flow->ssh_tls.ja4_client) == JA4_BYTE_COUNT, "SNI Hash key size differs changes since it was made. fix it");
#endif	
#ifdef _APP_MODE_BUILD_DB
			on_new_entry_found_db_gen(flow->ssh_tls.ja4_client, sizeof(flow->ssh_tls.ja4_client), flow->ssh_tls.notAfter);
#else
			on_new_entry_found_report(pktparams, flow->ssh_tls.ja4_client);
#endif
		}
	}
