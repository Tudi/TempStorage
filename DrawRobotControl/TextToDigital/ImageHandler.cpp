...
{
	return GenericWriter(dib, FileName, BMP_DEFAULT);
}

void SaveImage(BYTE* Pixels, int Width, int Height, int pitch, const char* FileName)
{
	FIBITMAP* dib2 = FreeImage_Allocate(Width, Height, 24);
	int pitch2 = FreeImage_GetPitch(dib2);
	BYTE* Pixels2 = FreeImage_GetBits(dib2);
	int bpp = FreeImage_GetBPP(dib2);
	...
	FreeImage_Save(FIF_BMP, dib2, FileName, BMP_DEFAULT);
}
